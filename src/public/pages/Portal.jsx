...
const PORTALS = {
  student: {
    label:         "Student",
    emoji:         <img src={student} className="w-5" alt="student" />,
    idLabel:       "Registration Number",
    idPlaceholder: "e.g. RGS/2024/001",
    idHint:        "Given to you by the school",
  ...
  parent: {
    ...
    emoji:         <img src={family} className="w-5" alt="parent" />,
    idLabel:       "Child's Registration Number",
    idPlaceholder: "e.g. RGS/2024/001",
    idHint:        "Use your child's registration number",
...
export default function Portal() {
  ...
  // If already logged in as student/parent redirect
  useEffect(() => {
  ...
  const switchTab = (t) => {
    setTab(t); setId(""); setPin(""); setError(""); setShowPin(false);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (pin.length < 4)            { triggerError("PIN must be at least 4 characters."); return; }

    setLoading(true);
    try {
      const res  = await fetch(`${API}/api/pins/login`, {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({
          regNumber: id.trim().toUpperCase(),
      ...
      // Store token separately from admin token
      localStorage.setItem("portalToken", data.token);
      localStorage.setItem("portalRole",  data.role);
      localStorage.setItem("portalStudent", JSON.stringify(data.student));
    ...
    <div className="pt-[6rem]">
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#c4a7c4] via-[#fff7ff] to-[#e8cfe8] flex items-center justify-center p-6 relative overflow-hidden font-sans">
        ...
        <div className={`relative z-10 w-full max-w-5xl flex flex-col lg:flex-row gap-6 items-stretch transition-all duration-700 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"}`}>
          ...
          {/* ── LEFT PANEL ── */}
          <div className="flex-1 flex flex-col gap-5 p-9 rounded-3xl text-white shadow-2xl"
            style={{ backgroundImage: `url(${checkresultimg})`, backgroundPosition: "center", backgroundSize: "cover" }}>
              ...
              {Object.entries(PORTALS).map(([key, p]) => (
                <button key={key} onClick={() => switchTab(key)}
                  className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-2xl border-2 font-semibold text-sm transition-all duration-300 cursor-pointer
                    ${tab === key ? p.tabColor : p.tabInactive}`}>
            ...
            <div className={`bg-white rounded-3xl p-8 shadow-xl border border-[#A033A0]/10 flex flex-col gap-6 transition-all duration-300 ${shake ? "animate-[shake_0.5s_ease]" : ""}`}
              ...
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shrink-0"
                  style={{ background: "#f5eaf5", border: `1.5px solid ${portal.accent}33` }}>
                  {portal.emoji}
                </div>
                <div>
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="relative flex items-center">
                    ...
                    <button type="button" onClick={() => setShowPin(p => !p)}
                      className="absolute right-3.5 text-gray-400 hover:text-gray-600 transition-colors cursor-pointer bg-transparent border-none">
                ...
                <button type="submit" disabled={loading}
                  ...
                  style={{ background: `linear-gradient(135deg, ${portal.accent}, #6d1b6d)` }}>
                  {loading ? (
