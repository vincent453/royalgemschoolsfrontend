...
const StatCard = ({ icon, label, value, sub, color, onClick }) => (
  <button onClick={onClick}
    className={`bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex items-center gap-4 text-left w-full
...
export default function InventoryDashboard() {
  ...
  useEffect(() => {
    getInventoryReport().then(setReport).catch(e=>setError(e.message)).finally(()=>setLoading(false));
  }, []);
        ...
        <main className="w-full overflow-y-auto">
          <div className="max-w-6xl mx-auto p-6 space-y-6">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              ...
              <p className="font-dm-sans text-sm text-gray-400 mt-1">Overview of all school assets and stock levels.</p>
            </div>
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard icon={<FaBoxes className="text-[#f056f0]" />} label="Total Items" value={report.totalItems}
                  sub={`Value: ${fmt(report.totalValue)}`} color="bg-pink-50" onClick={()=>navigate("/admin/inventory/items")} />
                <StatCard icon={<FaBoxes className="text-emerald-500" />} label="In Stock" value={report.inStock}
                  color="bg-emerald-50" onClick={()=>navigate("/admin/inventory/items?status=In Stock")} />
                <StatCard icon={<FaExclamationTriangle className="text-amber-500" />} label="Low Stock" value={report.lowStock}
                  color="bg-amber-50" onClick={()=>navigate("/admin/inventory/alerts")} />
                <StatCard icon={<FaTimesCircle className="text-red-500" />} label="Out of Stock" value={report.outOfStock}
                  color="bg-red-50" onClick={()=>navigate("/admin/inventory/alerts")} />
              </div>
            )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                { label:"Stock Movement", href:"/admin/inventory/stock-movement",  color:"bg-amber-500 text-white" },
              ].map(a=>(
                <button key={a.label} onClick={()=>navigate(a.href)}
                  className={`${a.color} rounded-2xl p-4 font-jost font-semibold text-sm hover:opacity-90 transition-opacity shadow-sm text-center`}>
                  {a.label}
                </button>
              ))}
            </div>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full text-sm">
                    <thead className="bg-gray-50 border-b border-gray-100">
                      <tr>
                        {["Date","Item","Type","Before","Change","After"].map(h=>(
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {report.recentMovements.map(m=>(
                        <tr key={m._id} className="hover:bg-[#fdf8ff] transition-colors">
                          ...
                          <td className={`px-5 py-3 font-jost font-bold ${m.type==="Stock In"||m.type==="Returned"?"text-emerald-600":"text-red-500"}`}>
