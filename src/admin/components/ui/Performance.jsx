...
const Performance = () => {
  ...
  useEffect(() => {
    const fetchPerformance = async () => {
      try {
        ...
        const averages = labels.map(cls => parseFloat((classMap[cls].total / classMap[cls].count).toFixed(1)));
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            ...
            padding: 10,
            cornerRadius: 10,
            callbacks: { label: ctx => `Avg: ${ctx.parsed.y}%` },
          },
        },
        scales: {
          x: { grid: { display: false }, ticks: { color: "#94a3b8", font: { size: 11 } }, border: { display: false } },
          y: { grid: { color: "#f1f5f9" }, ticks: { color: "#94a3b8", font: { size: 11 }, callback: v => `${v}%` }, border: { display: false }, min: 0, max: 100 },
        },
      },
    });
  ...
  const overallAvg = chartData.averages.length ? (totalResults / chartData.averages.length).toFixed(1) : "0";
