...
export const FinanceRoute = ({ children }) => {
  ...
  if (!["admin", "accountant"].includes(getRole())) {
    return <Navigate to={getRoleHome(getRole())} replace />;
  }
...
// ── Admin + Inventory Manager + Accountant ──
// Used for Shop access
export const ShopRoute = ({ children }) => {
  ...
  if (!["admin", "inventory_manager", "accountant"].includes(getRole())) {
    return <Navigate to={getRoleHome(getRole())} replace />;
  }
