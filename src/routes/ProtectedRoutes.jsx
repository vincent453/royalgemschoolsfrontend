...
// ── Shop management — admin + inventory manager ──
// Covers products/categories/orders (fulfillment). Dashboard stats,
// sales report, and customer spending stay admin-only at the API
// level (see ShopRoutes.js), so keep those specific pages wrapped in
// AdminRoute rather than this guard even if they live under /shop.
export const ShopRoute = ({ children }) => {
  const token = localStorage.getItem("token");
  if (!token) return <Navigate to="/admin/portal" replace />;
  if (!["admin", "inventory_manager"].includes(getRole())) {
    return <Navigate to={getRoleHome(getRole())} replace />;
  }
  return children;
};
